...
%Summarizing what we have seen, our aim is to give 
%automatic proofs for the decidable safety properties 
%of our programs. Before delving in how to do this 
%we should first formalize the notion of safety: 
%a given state is safe if, for any state reachable 
%in the Kleene closure of the step transition, 
%there is a successor state.
%\begin{align*}
%    &\mathsf{safe-state}(r.m)=\\
%    &\forall r'. m'. \quad (r.m\mapsto^{\ast}r'.m') 
%    \implies 
%    \exists r''.m'' \quad (r'.m'\mapsto %r''.m'')
%\end{align*}

%We say that a program is safe if, independently of 
%where we load it in memory, we end up with a safe sate. 
%Furthermore we need some safety conditions in order 
%to know if we have successor states or not. Some 
%examples may be:
%\begin{itemize}
%    \item $\forall v. (v\geq 50)\implies readable(v)$
%    
%\end{itemize}
